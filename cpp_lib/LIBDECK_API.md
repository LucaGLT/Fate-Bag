...
- [API Reference](#api-reference)
  ...
  - [TokenDeck Class](#tokendeck-class)
...
- **TokenDeck** manages **only** the deck structure (ordering, state, operations)
...
namespace FateBag {
    class TokenDeck {
    public:
        explicit TokenDeck(const std::vector<uint32_t>& token_ids, 
...
TokenDeck deck2(tokens, 42);

// Reproduce exact shuffle
TokenDeck deck3(tokens, 42);  // Same order as deck2
```
...
int main() {
    ...
    TokenDeck deck(card_ids, 42);  // seed=42 for reproducible tests
...
int main() {
    ...
    TokenDeck deck(tokens, 123);  // Fixed seed
...
Each thread creates its own `TokenDeck` instance (no contention).
...
- ✅ Core TokenDeck implementation with uint32_t optimization
