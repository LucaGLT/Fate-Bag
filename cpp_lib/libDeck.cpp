...
TokenDeck::TokenDeck(const std::vector<uint32_t>& token_ids, 
...
void TokenDeck::_validate_token_ids(const std::vector<uint32_t>& token_ids) {
...
void TokenDeck::shuffle() {
    std::shuffle(_deck.begin(), _deck.end(), _rng);
}

uint32_t TokenDeck::draw_one() {
    if (is_empty()) {
...
std::vector<uint32_t> TokenDeck::draw_many(int k) {
    if (k <= 0) {
...
int TokenDeck::remaining_count() const {
    return static_cast<int>(_deck.size());
}

bool TokenDeck::is_empty() const {
    return _deck.empty();
}

void TokenDeck::reset(const std::optional<std::vector<uint32_t>>& token_ids) {
...
std::vector<uint32_t> TokenDeck::peek_all() const {
    return std::vector<uint32_t>(_deck);
}

void TokenDeck::remove(uint32_t token_id) {
...
bool TokenDeck::contains(uint32_t token_id) const {
